...
if ($vip_exp<100&&$room_book_number>3){
    ...
    for ($i=0;$i<$room_book_number;$i++) {
        mysqli_query($myConn, "insert into room_time(room_id,begin_date,end_date)VALUE ($room_id,'$begin_date','$end_date')");
    }

    $result=mysqli_query($conn,"select hotel_id from room WHERE room_id=$room_id ");
    $result_arr=mysqli_fetch_assoc($result);
    $hotel_id=$result_arr['hotel_id'];
    mysqli_query($myConn,"update hotel set hot=hot+1 WHERE hotel_id=$hotel_id");
