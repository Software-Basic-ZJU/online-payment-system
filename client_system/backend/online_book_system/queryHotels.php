    ...
    class query{
        public static function queryHotels($place,$star_option,$price_option,$score_option)
        {
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                $result2=mysqli_query($myConn,"select room_type,user_discount,price from room WHERE hotel_id=$hotel_id and user_discount<100 and amount >0");
                while ($row2=mysqli_fetch_assoc($result2)){
                    $price=$row2['price'];$user_discount=$row2['user_discount'];
                    $nowPrice=$price*$user_discount;
                    $res2[$j++]=array(
