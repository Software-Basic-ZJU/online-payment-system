...
class refundApc{
    private static $oside;
    public static function refundOrder($orderID,$reason)
    {
        ...
        if($flag==0){
            ...
            mysqli_query($myConn,"update order_records set state=6,action_time='$time' where order_id='$orderID'");
            //add it to the refund table
            mysqli_query($myConn,"insert into refund (buyer,order_id,reason,state) values ('$buyer','$orderID','$reason','$state')");
            ...
            mysqli_query($myConn,"insert into transact_flow (event_id,type,event_time,state) values('$eventID','1','$time','$state')");
...
if(checkStringSafety($orderID))
    if(checkStringSafety($reason)){
        refundApc::refundOrder($orderID,$reason);
        $title="交易状态改变";
        $body="用户申请订单退款";
        $user_id=refundApc::getOside();
